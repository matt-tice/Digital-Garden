...
(defun my-cloud-export (infile outfile)
  "Convert INFILE to OUTFILE by driving Pandoc via deterministic file abstractions."
  (let* ((abs-infile (expand-file-name infile))
    ...
    (with-current-buffer (find-file-noselect abs-infile)
      (let ((org-export-filter-parse-tree-functions '(my/org-export-private-to-stub))
    ...
    (let* ((pandoc-cmd (format "pandoc -f org -t gfm -o %s %s" 
                               (shell-quote-argument abs-outfile) 
